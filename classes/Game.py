...
class Game:
    ...
    def game_over(self):
        ...
        while self.isfinished:
            ...
            self.screen.msg_display('GAME OVER', self.screen.width / 2,
                                    self.screen.height / 4, 100)
            # add points to screen
            self.screen.msg_display(u'Points: ' + str(self.points),
                                    self.screen.width / 2,
                                    self.screen.height / 2, 40, Colors.blue)
            # add button to go back to main menu
            self.screen.button('Go To Main', 580, 520, 180, 40, Colors.blue,
                               Colors.bright_blue, Colors.white,
                               self.show_mainmenu)
    ...
    def game_highscore(self):
        ...
        while not done:
            ...
            for event in pygame.event.get():
                ...
                if event.type == KEYDOWN:
                    if event.key == K_RETURN:
                        self.highscore.add_highscore(self.player, self.points)
                        done = True
            ...
            self.screen.button('', self.screen.width / 2 - 190,
                               self.screen.height / 2 - 35, 400, 100,
                               Colors.grey, Colors.grey, Colors.grey)
            # add button as input-box where the player inputs his name
            self.screen.button(self.player, self.screen.width / 2 - 200,
                               self.screen.height / 2 - 50, 400, 100,
    ...
    def show_options(self):
        ...
        while self.options_isshown:
            ...
            self.screen.fill(Colors.white)
            # add headline
            self.screen.msg_display('Options', self.screen.width / 2,
                                    self.screen.height / 5, 100)
            ...
            self.screen.button(f'Game-Level: {btn_game_level_text}',
                               self.screen.width / 2 - button_width / 2,
                               menu_y_start + button_height + menu_y_gap,
                               button_width, button_height, Colors.blue,
                               Colors.bright_blue, Colors.white)

            # add go_back-Button
            self.screen.button('Go Back', 580, 520, 180, 40, Colors.blue,
                               Colors.bright_blue, Colors.white,
                               self.exit_options)
    ...
    def show_help(self):
        ...
        while self.help_isshown:
            ...
            self.screen.fill(Colors.white)
            # add headline
            self.screen.msg_display('Help', self.screen.width / 2,
                                    self.screen.height / 5, 100)
            # add text to display keyboard functions
            self.screen.msg_display('[P] --> Pause, take a break',
                                    self.screen.width / 2, top_anchor, 20)
            self.screen.msg_display('[M] --> Mute sounds',
                                    self.screen.width / 2,
                                    top_anchor + top_anchor_gap, 20)
            self.screen.msg_display('[ESCB] --> Go Back or Quit Game',
                                    self.screen.width / 2,
                                    top_anchor + top_anchor_gap * 2, 20)
    ...
    def game_pause(self):
        ...
        while self.pause:
            ...
            # add headline
            self.screen.msg_display('PAUSED', self.screen.width / 2,
                                    self.screen.height / 4, 100)
            # add continue-Button
            self.screen.button('Continue', 300, 270, 200, 50, Colors.blue,
                               Colors.bright_blue, Colors.white,
                               self.game_continue)
    ...
    def show_mainmenu(self):
        ...
        while menu_isshown:
            ...
            # add headline
            self.screen.msg_display("Hit the mole!", self.screen.width / 2,
                                    self.screen.height / 5, 100)
            # add new_game-Button
            self.screen.button(
                'New Game',
            ...
            self.screen.button(
                'Options', self.screen.width / 2 - button_width / 2,
                menu_y_start + button_height * 2 + menu_y_gap * 2,
                button_width, button_height, Colors.orange,
                Colors.bright_orange, Colors.white, self.show_options)
            # add show_help-Button
            self.screen.button(
                'Help', self.screen.width / 2 - button_width / 2,
                menu_y_start + button_height * 3 + menu_y_gap * 3,
                button_width, button_height, Colors.violett,
                Colors.bright_violett, Colors.white, self.show_help)
            # add quit_game-Button
            self.screen.button(
                'Quit Game', self.screen.width / 2 - button_width / 2,
                menu_y_start + button_height * 4 + menu_y_gap * 4,
                button_width, button_height, Colors.red, Colors.bright_red,
                Colors.white, self.game_quit)
        ...
        while running:
            ...
            for event in pygame.event.get():
                if event.type == QUIT:
                    ...
                    self.screen.msg_hud_left(
                        u'Fehler: ' + str(self.count_fails), 15,
                        self.screen.height - 45, 18, Colors.white)
                    self.screen.msg_hud_left(u'Point: ' + str(self.points),
                                             15, self.screen.height - 15, 18,
                                             Colors.white)
                    break
                elif event.type == KEYDOWN:
                    if event.key == K_p:
